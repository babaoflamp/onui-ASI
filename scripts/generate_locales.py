...
EXISTING_LANGS = ['ko', 'en', 'ja', 'zh']
NEW_LANGS = ['vi', 'uz', 'mn', 'ne', 'my', 'id', 'th', 'fr']
...
def get_file_content(filepath):
    ...
    try:
        ...
        if result.returncode == 0 and ('GLOBAL_I18N' in result.stdout or 'PAGE_I18N' in result.stdout):
...
def extract_i18n():
    ...
    for filepath in FILES_TO_PARSE:
        ...
        if not content:
            print(f"  [건너뜀] {filepath} (파일을 읽을 수 없거나 I18N 데이터가 없습니다)")
            continue
            
        # GLOBAL_I18N 또는 PAGE_I18N 블록 찾기
        blocks = re.findall(r'(?:GLOBAL_I18N|PAGE_I18N)\s*=\s*{(.*?)};', content, re.DOTALL)
    ...
    total_extracted = len(locales['ko'])
    ...
    # 새로운 6개 국어 생성 (영어 데이터를 기준으로 기본 생성해 둠)
    fallback_data = locales['en'] if locales['en'] else locales['ko']
    for lang in NEW_LANGS:
        file_path = out_dir / f"{lang}.json"
        with open(file_path, "w", encoding="utf-8") as f:
            json.dump(fallback_data, f, ensure_ascii=False, indent=2)
        print(f"✅ {file_path} 파일 생성 완료 (영어/한국어 Fallback 복사)")
